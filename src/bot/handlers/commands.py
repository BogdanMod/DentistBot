...
def _cost_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="Ортодонтическое/Гнатологическое лечение", callback_data="cost_ortho")],
            [InlineKeyboardButton(text="Элайнеры", callback_data="cost_aligners")],
            [InlineKeyboardButton(text="Брекеты", callback_data="cost_braces")],
...
async def cost_item(callback) -> None:
    await callback.answer()
    pricing = {
        "cost_ortho": ("Ортодонтическое/Гнатологическое лечение", "Средняя стоимость: 180.000₽ - 320.000₽"),
        "cost_aligners": ("Элайнеры", "Средняя стоимость: 220.000₽ - 380.000₽"),
        "cost_braces": ("Брекет-система", "Средняя стоимость: 210.000₽ - 250.000₽"),
        "cost_implant": ("Имплантация", "Средняя стоимость: 65.000₽ - 120.000₽ за единицу"),
        "cost_veneers": ("Виниры и протезирование", "Средняя стоимость: 35.000₽ - 85.000₽ за единицу"),
        "cost_therapy": ("Лечение зубов", "Средняя стоимость: 8.000₽ - 25.000₽"),
        "cost_hygiene": ("Профессиональная гигиена", "Средняя стоимость: 7.000₽ - 15.000₽"),
    }
    title, price = pricing.get(callback.data, ("Лечение", "Стоимость уточняется"))
    await callback.message.answer(
        f"{title}\n"
        f"{price}\n"
        "Точная стоимость определяется после диагностики и составления плана лечения.",
